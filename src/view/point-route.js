...
import { getDaysPoint, getHoursPoint, getMinutesPoint} from '../utils/time';
import he from 'he';
...
const getDuration = (dateFrom, dateTo) => {
  ...
  const days = Math.trunc(difference / 1440);
  const hours = Math.trunc((difference - days * 1440) / 60);
  const minutes = difference - (days * 1440 + hours * 60);

  const daysOutput = getDaysPoint(days);
  const hoursOutput = getHoursPoint(days, hours);
  const minutesOutput = getMinutesPoint(minutes);
...
export default class PointRoute extends AbstractView {
  ...
  get template() {
    return createPointRouteTemplate(this.#point, this.#destination, this.#offers);
  }
